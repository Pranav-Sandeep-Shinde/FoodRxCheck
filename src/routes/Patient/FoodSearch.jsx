                ...
                {searchTerm && Object.keys(groupedDrugs).length > 0 && (
                    <div>
                        ...
                        {Object.keys(groupedDrugs).map((letter) => (
                            ...
                            <h2 className="text-xl md:text-2xl font-semibold text-gray-700 mb-4 border-b-2 border-gray-300 pb-2">
                                {letter}
                            </h2>
                            <motion.div
                            ...
                            >
                                {groupedDrugs[letter].map(({ drug_id, drug_name }) => (
                                    <motion.div
                                        key={drug_id}
                                        whileHover={{ scale: 1.05 }}
                                        whileTap={{ scale: 0.98 }}
                                        className="p-4 sm:p-5 lg:p-6 bg-white rounded-xl shadow-xl cursor-pointer hover:shadow-2xl transition-all duration-300"
                                        onClick={() => navigate(`/drug-interaction/${drug_id}/${drug_name}`)}
                                    >
                                        <p className="text-lg sm:text-xl font-semibold text-gray-800">{drug_name}</p>
                                    </motion.div>
                                ))}
                            </motion.div>
                    </div>
