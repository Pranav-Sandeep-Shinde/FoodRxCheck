...
export default function DrugCarousel() {
  ...
  return (
    <div className="flex flex-col items-center justify-center min-h-screen bg-white py-10 relative">
      ...
      {/* 🏷️ Page Title */}
      <h1 className="text-3xl font-bold px-4 py-2 transition-all duration-300 bg-teal-600 text-white rounded-md">
  Explore Drugs
</h1>
  ...
  <div className="grid grid-cols-2 md:grid-cols-3  gap-4 px-6">
    {filteredDrugs.map((drug) => (
      ...
      >
        ...
        {/* Action Button */}
        <button
          className={`absolute top-4 right-4 text-gray-700 hover:text-green-500 ${
            selectedDrugs.some(({ drug_id }) => drug_id === drug.drug_id) ? "text-green-500" : ""
          }`}
          onClick={(e) => {
            e.stopPropagation();
