...
import { ArrowRight, Shield, Heart, Clock, UserCircle2, LogOut, ClipboardCheck, TrendingUp, Database, Thermometer } from 'lucide-react';
  ...
  return (
    <>
      {role === 'patient' ?
        (
          <div
            className={`max-w-full md:px-28 px-4 py-8 md:ml-20 transition-all duration-300 ${showHero ? 'opacity-100' : 'opacity-0'
              }`}
          >
            {/* Hero Section */}
            <div className="text-center mb-16 relative">
              ...
              <button
                className={`inline-flex items-center px-6 py-3 bg-${themeColor}-600 text-white rounded-lg hover:${themeColor}-700 transition-colors`}
            ...
            >
              ...
              <span className="hidden md:inline">{user?.full_name.split(' ')[0] || 'Profile'}</span>
                ...
                <button
                  className={`fixed bottom-10 right-10 flex items-center justify-center w-14 h-14 rounded-full bg-${themeColor}-600/80 backdrop-blur-md text-white shadow-lg transition-all duration-300 hover:bg-${themeColor}-700 active:scale-90 active:shadow-md`}
        ...
        (
          ...
          >
            ...
            <div className="text-center mb-16 relative">
              ...
              <button
                className={`inline-flex items-center px-6 py-3 bg-${themeColor}-600 text-white rounded-lg hover:bg-${themeColor}-700 transition-colors`}
                onClick={() => navigate('/hcpdruglist')}
              >
                View DrugList and Interaction
                <ArrowRight className="ml-2 h-5 w-5" />
              </button>
              ...
              <span className="hidden md:inline">{user?.full_name.split(' ')[0] || 'Profile'}</span>
                  ...
                  {showProfile && (
                    <div className="absolute right-0 mt-2 w-64 bg-white shadow-lg rounded-lg p-4 text-gray-900 z-50">
                      ...
                      {qualificationData && (
                        <div className="p-4 bg-gray-100 rounded-lg shadow-md">
                          <h5 className={`text-lg font-bold text-${themeColor}-500 text-center mb-2`}>Qualification Details</h5>
                          <p className="text-gray-700 text-sm"><span className="font-semibold">Degree: </span>{qualificationData.degree}</p>
                          <p className="text-gray-700 text-sm"><span className="font-semibold">Department: </span>{qualificationData.department}</p>
                          <p className="text-gray-700 text-sm"><span className="font-semibold">Institution: </span>{qualificationData.institution}</p>
                        </div>
            ...
            <div className="grid md:grid-cols-3 gap-8 mb-16">
              <FeatureCard
                ...
                description="Evidence-based guidelines reviewed by medical experts"
                themeColor={themeColor}
              />
              <FeatureCard
                icon={TrendingUp}
                title="Efficiency & Workflow"
                description="Streamlined instructions to enhance patient care"
                themeColor={themeColor}
              />
              <FeatureCard
                icon={Database}
                title="Comprehensive Resource"
                description="Instant access to up-to-date medical protocols"
                themeColor={themeColor}
              />
            </div>
...
const FeatureCard = ({ icon: Icon, title, description, themeColor }) => (

  <div className="bg-white p-6 rounded-xl shadow-md hover:shadow-lg transition-shadow">
    <div className={`bg-${themeColor}-100 w-12 h-12 rounded-lg flex items-center justify-center mb-4`}>
